...
parser.add_argument('--s_init', type=str, default='./results/model_stu.pth.tar', help='initial parameters of student model')
parser.add_argument('--t_model', type=str, default='./results/model_tea.pth.tar', help='path name of teacher model')
...
parser.add_argument('--t_name', type=str, default='resnet20', help='name of teacher')    # resnet20/resnet110
parser.add_argument('--s_name', type=str, default='resnet20', help='name of student')    # resnet20/resnet110

# hyperparameter
parser.add_argument('--kd_mode', type=str, default='at', help='mode of kd, which can be:'
...
fh = logging.FileHandler(os.path.join(args.save_root, 'log_kd.txt'))
...
def main():
	...
	test_loader_1 = datasets.MNIST('./datasets', train=False, transform=transforms.Compose([
        transforms.Pad(4, padding_mode='reflect'),
        transforms.ToTensor(),
	...
	if args.kd_mode in ['fsp', 'ab']:
		...
		train_init(train_loader, nets, optimizer, criterions, 10)   ###50
		args.lambda_kd = 0.0
	...
	for epoch in range(1, args.epochs+1):
		...
		test_top1, test_top5 = test(test_loader, nets, criterions, epoch, test_loader_1)
	...
	save_path = os.path.join(save_root, 'model_stu_final.pth.tar')
...
def test(test_loader, nets, criterions, epoch, test_loader_1):
	cls_losses = AverageMeter()
	kd_losses  = AverageMeter()
	top1       = AverageMeter()
	top5       = AverageMeter()
	...
	test_inall(test_loader, snet)
	test_5_9(test_loader_1, snet)


	return top1.avg, top5.avg

def test_inall(test_loader, model):
	...
	for data, target in test_loader:
		...
		temp = target.data.view_as(pred)
		temp1 = pred.eq(temp.cuda()).cuda()
		cnt = temp1.sum()
		correct += cnt  # 对预测正确的数据个数进行累加
...
def test_5_9(test_loader, model):
	testx_2 = []
	testy_2 = []
	num_images = 100
	...
	temp = (result.argmax(dim=1).cuda() == testy_2.cuda())
	accu = temp.cuda().sum().item() / len(testx_2)
