...
parser = argparse.ArgumentParser(description='train teacher net')
...
def main():
	...
	save_path = os.path.join(args.save_root, 'model_tea.pth.tar')
	torch.save({
		'epoch': 0,
		'net': net.state_dict(),
		'prec@1': 0.0,
...
def save_(state, is_best, save_root):
	save_path = os.path.join(save_root, 'model_tea_point.pth.tar')
	torch.save(state, save_path)
	if is_best:
		best_save_path = os.path.join(save_root, 'model_tea.pth.tar')
